...
// Update the pending orders query to include all relevant pending statuses
$sql_select_order = "SELECT * FROM `order` WHERE `status`='placed' OR `status`='Pending' OR `status`='pending'";
$data_order = mysqli_query($conn,$sql_select_order);
$order_count = mysqli_num_rows($data_order);

// Update delivered orders query to standardize status check
$sql_select_order_deli = "SELECT * FROM `order` WHERE `status`='Delivered' OR `status`='delivered'";
$data_order_deli = mysqli_query($conn,$sql_select_order_deli);
$order_count_deli = mysqli_num_rows($data_order_deli);

// Update cancelled orders query
$sql_select_order_can = "SELECT * FROM `order` WHERE 
    `status` IN ('Cancelled', 'cancelled', 'Cancelled-By-Supplier', 'Cancelled-By-Client')";
$data_order_can = mysqli_query($conn,$sql_select_order_can);
$order_count_can = mysqli_num_rows($data_order_can);

// Add error checking
if (!$data_order || !$data_order_deli || !$data_order_can) {
