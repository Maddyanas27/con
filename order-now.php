...
if (isset($_POST['buy'])) {
    ...
    // First insert order with status 'placed'
    $sql_insert = "INSERT INTO `order` (`product_id`, `user_id`, `name`, `price`, `num_product`, `image`, `status`) 
                   SELECT `product_id`, `user_id`, `name`, `price`, `num_product`, `image`, 'placed' 
                   FROM `cart` WHERE `user_id`='$login_id'";
    ...
    if(mysqli_query($conn, $sql_insert)) {
        ...
        $sql_update = "UPDATE `order` SET 
                      ...
                      `date_time`='$date_time',
                      `payment`='$payment'
                      WHERE `user_id`='$login_id' AND `status`='placed'";
...
// In your order processing section:
if(isset($_SESSION['login'])) {
    $login_id = $_SESSION['login'];
    
    // Get cart items
    $sql_cart = "SELECT product_id, num_product FROM cart WHERE user_id = ?";
    $stmt_cart = mysqli_prepare($conn, $sql_cart);
    mysqli_stmt_bind_param($stmt_cart, "i", $login_id);
    ...
    try {
        while($cart_item = mysqli_fetch_assoc($result_cart)) {
            ...
            $sql_check = "SELECT stock_count FROM product WHERE id = ?";
            $stmt_check = mysqli_prepare($conn, $sql_check);
            mysqli_stmt_bind_param($stmt_check, "i", $cart_item['product_id']);
            mysqli_stmt_execute($stmt_check);
            $result_check = mysqli_stmt_get_result($stmt_check);
            $product = mysqli_fetch_assoc($result_check);
            
            if($product['stock_count'] < $cart_item['num_product']) {
                $stock_error = true;
                throw new Exception("Not enough stock available");
            }
            
            // Update stock
            if(!updateProductStock($conn, $cart_item['product_id'], $cart_item['num_product'])) {
                throw new Exception("Error updating stock");
            }
        }
        
        // Process order (your existing order creation code)
        // ...existing order processing code...
        
        // Clear cart
        $sql_clear = "DELETE FROM cart WHERE user_id = ?";
        $stmt_clear = mysqli_prepare($conn, $sql_clear);
        mysqli_stmt_bind_param($stmt_clear, "i", $login_id);
