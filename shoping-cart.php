...
function checkStockAvailability($conn, $cart_id) {
    $sql = "SELECT p.stock_count, c.num_product 
            FROM cart c 
    ...
    return $row['stock_count'] >= $row['num_product'];
}
...
$sql_alter_table = "ALTER TABLE `product` ADD COLUMN `stock_count` INT DEFAULT 0;";
mysqli_query($conn, $sql_alter_table);

// Add this to create the shipping_address table
$sql_create_shipping_address_table = "CREATE TABLE IF NOT EXISTS `shipping_address` (
...
mysqli_query($conn, $sql_create_shipping_address_table);
	...
	<header class="header-v4">
		...
		<div class="wrap-header-mobile">
			...
			<div class="logo-mobile">
				<a href="index.php"><img src="images/icons/log.jpg" alt="IMG-LOGO"></a>
			</div>
			...
			<div class="row">
								...
								<tr class="table_row">
									...
									<td class="column-4">
										<div class="wrap-num-product flex-w m-l-auto m-r-0">
											<a href="javascript:void(0)" 
											   class="btn-num-product-down cl8 hov-btn3 trans-04 flex-c-m decrease_number" 
											   attr_id="<?php echo $row['id']; ?>" 
											   attr_pro="<?php echo max(1, $row['num_product']-1); ?>">
											...
											<input class="mtext-104 cl3 txt-center num-product" 
												   type="number" 
												   name="num-product1" 
												   value="<?php echo $row['num_product']; ?>" 
												   readonly>

											<a href="javascript:void(0)" 
											   class="btn-num-product-up cl8 hov-btn3 trans-04 flex-c-m increase_number" 
											   attr_id="<?php echo $row['id']; ?>" 
											   attr_pro="<?php echo $row['num_product']+1; ?>">
				...
				<div class="col-sm-12 col-lg-7 col-xl-5 m-lr-auto m-b-50">
					<div class="bor10 p-lr-40 p-t-30 p-b-40 m-l-63 m-r-40 m-lr-0-xl p-lr-15-sm">
						...
						<?php 
						// Add this before the "Proceed to Buy" button
						$all_items_available = true;
						if(isset($_SESSION['login'])) {
						    $sql_check = "SELECT c.id 
						                  FROM cart c 
						                  JOIN product p ON c.product_id = p.id 
						                  WHERE c.user_id = ? AND c.num_product > p.stock_count";
						    $stmt_check = mysqli_prepare($conn, $sql_check);
						    mysqli_stmt_bind_param($stmt_check, "i", $login_id);
						...
						<button class="flex-c-m stext-101 cl0 size-116 bg3 bor14 hov-btn3 p-lr-15 trans-04 pointer 
						    ...
						    name="buy" 
						    onclick="window.location.href='shipping-address.php'" 
						    <?php echo !$all_items_available ? 'disabled' : ''; ?>>
						    <?php echo $all_items_available ? 'Proceed to Buy' : 'Some items out of stock'; ?>
						</button>
